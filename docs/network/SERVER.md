...
Příkaz `backup` potřebuje nainstalovaný `pg_dump` a `pg_restore` kompatibilní se serverem v PATH. Server ani tyto administrátorské programy nejsou součástí klientského ZIP. Zálohy upraveného pilotu vyžadují `--allow-pilot-changes`. Zálohu ověřujte obnovou do jiné testovací databáze; role a hesla je nutné spravovat samostatně. Denní/týdenní zálohování a jeho cílové úložiště nastaví správce na firemní infrastruktuře.
