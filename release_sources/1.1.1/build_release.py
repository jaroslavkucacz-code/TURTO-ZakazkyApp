...
BASE_SOURCE_URL = 'https://raw.githubusercontent.com/jaroslavkucacz-code/TURTO-ZakazkyApp/e4498af30f65b7f37d86710a387130925714a8e8/releases/1.1.0/TURTO_source_1.1.0.zip'
BASE_SOURCE_SHA = '21d06963eb25531c5aefcfd93d9765a9b00030fea040c500631b940f4a8301f0'
...
def run() -> None:
    ...
    base_zip=pathlib.Path('_base_source_111.zip')
    base=download(BASE_SOURCE_URL,BASE_SOURCE_SHA,base_zip)
    base_dir=pathlib.Path('_base_source_111');clean(base_dir);base_dir.mkdir()
    with zipfile.ZipFile(io.BytesIO(base)) as z:
        assert z.testzip() is None; z.extractall(base_dir)

    stage=pathlib.Path('_stage_111');clean(stage);shutil.copytree(base_dir,stage)
    ...
    patch_version(stage)
    note=(ROOT/'ZMENY_1.1.1.txt').read_text(encoding='utf-8')
    (stage/'ZMENY_1.1.1.txt').write_text(note,encoding='utf-8')
    ...
    qa=pathlib.Path('releases/1.1.1/validation').resolve();qa.mkdir(parents=True,exist_ok=True)
    ...
    source_stage=pathlib.Path('_source_111');clean(source_stage);shutil.copytree(stage,source_stage)
    clean_python_cache(source_stage)
    source_data=zip_deterministic(source_stage)
    pathlib.Path('releases/1.1.1/TURTO_source_1.1.1.zip').write_bytes(source_data)

    version_file=pathlib.Path('_turto_reporting_111_version.txt')
    version_file.write_text(
        "VSVersionInfo(ffi=FixedFileInfo(filevers=(1,1,1,0),prodvers=(1,1,1,0),mask=0x3f,flags=0x0,OS=0x40004,fileType=0x1,subtype=0x0,date=(0,0)),kids=[StringFileInfo([StringTable('040504B0',[StringStruct('CompanyName','TURTO s.r.o.'),StringStruct('FileDescription','TURTO – Měsíční přehledy'),StringStruct('FileVersion','1.1.1'),StringStruct('InternalName','TURTO_Mesicni_Prehledy'),StringStruct('OriginalFilename','TURTO_Mesicni_Prehledy.exe'),StringStruct('ProductName','TURTO – Měsíční přehledy'),StringStruct('ProductVersion','1.1.1')])]),VarFileInfo([VarStruct('Translation',[1029,1200])])])\n",
        encoding='utf-8')

    dist_main=pathlib.Path('_dist_main_111').resolve();work_main=pathlib.Path('_work_main_111').resolve();spec_main=pathlib.Path('_spec_main_111').resolve()
    ...
    smoke_data=pathlib.Path('_smoke_data_111').resolve();clean(smoke_data)
    ...
    install_root=pathlib.Path('_installed_111').resolve();test_data=pathlib.Path('_installed_data_111').resolve()
    ...
    upgrade_root=pathlib.Path('_upgrade_111').resolve();upgrade_data=pathlib.Path('_upgrade_data_111').resolve()
    ...
    update_stage=pathlib.Path('_update_stage_111');clean(update_stage);update_stage.mkdir()
    ...
    rspec=importlib.util.spec_from_file_location('release_safety_111',stage/'src/release_safety.py')
    ...
    print('TURTO_1_1_1_OK', 'update=',len(update_data),'setup=',setup.stat().st_size)
