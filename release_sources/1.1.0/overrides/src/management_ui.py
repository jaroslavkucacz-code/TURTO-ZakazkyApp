...
class ManagementUI:
    ...
    def _import_worker(self,title,fn,done):
        ...
        win.transient(self);win.grab_set();win.protocol('WM_DELETE_WINDOW',lambda:None)
    ...
    def _preview_dialog(self,batch):
        ...
        def commit():
            ...
            self._import_worker('Ukládám data a zálohu',lambda:apply_imports(self.db,batch,resolve_path(self.cfg['import_archive_dir']),
                                resolve_path(self.cfg['backup_dir']),replacing),finished)
