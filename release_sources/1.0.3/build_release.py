...
def run() -> None:
    ...
    base_zip=pathlib.Path('_base_source_103.zip')
    base=download(BASE_SOURCE_URL,BASE_SOURCE_SHA,base_zip)
    base_dir=pathlib.Path('_base_source_103');clean(base_dir);base_dir.mkdir()
    with zipfile.ZipFile(io.BytesIO(base)) as z:
        assert z.testzip() is None; z.extractall(base_dir)

    stage=pathlib.Path('_stage_103');clean(stage);shutil.copytree(base_dir,stage)
    ...
    patch_version(stage)
    note=(ROOT/'ZMENY_1.0.3.txt').read_text(encoding='utf-8')
    (stage/'ZMENY_1.0.3.txt').write_text(note,encoding='utf-8')
    ...
    version_file=pathlib.Path('_turto_reporting_103_version.txt')
    version_file.write_text(
        "VSVersionInfo(ffi=FixedFileInfo(filevers=(1,0,3,0),prodvers=(1,0,3,0),mask=0x3f,flags=0x0,OS=0x40004,fileType=0x1,subtype=0x0,date=(0,0)),kids=[StringFileInfo([StringTable('040504B0',[StringStruct('CompanyName','TURTO s.r.o.'),StringStruct('FileDescription','TURTO – Měsíční přehledy'),StringStruct('FileVersion','1.0.3'),StringStruct('InternalName','TURTO_Mesicni_Prehledy'),StringStruct('OriginalFilename','TURTO_Mesicni_Prehledy.exe'),StringStruct('ProductName','TURTO – Měsíční přehledy'),StringStruct('ProductVersion','1.0.3')])]),VarFileInfo([VarStruct('Translation',[1029,1200])])])\n",
        encoding='utf-8')

    dist_main=pathlib.Path('_dist_main_103').resolve();work_main=pathlib.Path('_work_main_103').resolve();spec_main=pathlib.Path('_spec_main_103').resolve()
    ...
    smoke_data=pathlib.Path('_smoke_data_103').resolve();clean(smoke_data)
    ...
    install_root=pathlib.Path('_installed_103').resolve();test_data=pathlib.Path('_installed_data_103').resolve()
    ...
    # Upgrade test: 1.0.1 obsahuje helper, 1.0.3 jej musí instalací odstranit.
    old_setup=pathlib.Path('_old_setup_101.exe')
    download(BASE_INSTALLER_URL,BASE_INSTALLER_SHA,old_setup)
    upgrade_root=pathlib.Path('_upgrade_103').resolve();upgrade_data=pathlib.Path('_upgrade_data_103').resolve()
    ...
    # Runtime ZIP zůstává publikovaný jako kompatibilní fallback, ale 1.0.3+ preferuje installer_url.
    update_stage=pathlib.Path('_update_stage_103');clean(update_stage);update_stage.mkdir()
    ...
    rspec=importlib.util.spec_from_file_location('release_safety_103',stage/'src/release_safety.py')
    ...
    pathlib.Path('_v103_update_sha.txt').write_text(hashlib.sha256(update_data).hexdigest(),encoding='ascii')
    pathlib.Path('_v103_setup_sha.txt').write_text(setup_sha,encoding='ascii')
    print('TURTO_1_0_3_OK', 'update=',len(update_data),'setup=',setup.stat().st_size)
