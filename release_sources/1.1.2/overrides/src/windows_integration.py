...
def _root_string(root) -> str:
    """Return a plain absolute string path without retaining pathlib objects in callbacks."""
    try:
...
def _set_property_store_strings(store_ptr, values) -> bool:
    ...
    class PVUNION(ctypes.Union):
        _fields_ = [('pwszVal', wintypes.LPWSTR), ('ptr', ctypes.c_void_p), ('storage', ctypes.c_ubyte * (16 if ctypes.sizeof(ctypes.c_void_p) == 8 else 8))]
    ...
    vtbl = ctypes.cast(store_ptr, ctypes.POINTER(ctypes.POINTER(ctypes.c_void_p))).contents
    ...
    fmtid = GUID.from_buffer_copy(uuid.UUID('9F4C2855-9F79-4B39-A8D0-E1D42DE1D5F3').bytes_le)
    VT_LPWSTR = 31
    ok = True
    buffers = []
    try:
        for pid, value in values:
            ...
            if SetValue(store_ptr, ctypes.byref(key), ctypes.byref(pv)) < 0:
                ok = False
...
def _set_window_app_properties(hwnd: int, root: Path) -> None:
    ...
    try:
        ...
        fn.argtypes = [wintypes.HWND, ctypes.c_void_p, ctypes.POINTER(ctypes.c_void_p)]
        fn.restype = ctypes.c_long
        iid = _guid('886D8EEB-8CF2-4446-8D02-CDBA1DBDCF99')
        store = ctypes.c_void_p()
        if fn(wintypes.HWND(hwnd), ctypes.byref(iid), ctypes.byref(store)) < 0 or not store.value:
            return
        # The installed shortcut provides the executable, icon and name.
        # Do not publish the legacy pythonw/app.pyw relaunch command in native builds.
...
def _set_shortcut_app_id(path: Path) -> None:
    ...
    try:
        ...
        fn.argtypes = [wintypes.LPCWSTR, ctypes.c_void_p, wintypes.DWORD, ctypes.c_void_p, ctypes.POINTER(ctypes.c_void_p)]
        fn.restype = ctypes.c_long
        iid = _guid('886D8EEB-8CF2-4446-8D02-CDBA1DBDCF99')
        store = ctypes.c_void_p()
        if fn(str(path), None, 0x00000002, ctypes.byref(iid), ctypes.byref(store)) >= 0 and store.value:
            _set_property_store_strings(store, [(5, APP_USER_MODEL_ID)])
    except BaseException:
        return
...
def apply_native_window_icon(window, root) -> None:
    """Best-effort Windows icon update. This function must never break the UI loop."""
    if sys.platform != 'win32':
        return
    try:
        root_text = _root_string(root)
        if not root_text:
            return
...
def install_native_icon_refresh(window, root) -> None:
    """Finite, guarded icon refresh. No Map/Visibility event loop bindings."""
    if sys.platform != 'win32':
        return
    root_text = _root_string(root)
    if not root_text:
    ...
    def safe_apply():
        state['pending'] = False
        try:
            apply_native_window_icon(window, root_text)
        except BaseException:
            pass

    def schedule(delay):
        try:
            window.after(delay, safe_apply)
        except BaseException:
            pass

    for delay in (0, 120, 500, 1500, 3000):
        schedule(delay)
...
def ensure_windows_shortcuts(root) -> None:
    ...
    try:
        ...
        repair_current_user(force=False)
    except Exception:
        # The settings action exposes detailed errors without preventing startup.
        return
