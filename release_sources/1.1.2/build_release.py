...
BASE_SOURCE_URL = 'https://raw.githubusercontent.com/jaroslavkucacz-code/TURTO-ZakazkyApp/e3b26668fc57f4dcec7b2b0d99ad861c1ce7aa70/releases/1.1.1/TURTO_source_1.1.1.zip'
BASE_SOURCE_SHA = 'f507d4748efd48aeff6d7f6ed1ded4c2c002b720cc4f6b830b6a6853fedbac7b'
...
def run() -> None:
    ...
    base_zip=pathlib.Path('_base_source_112.zip')
    base=download(BASE_SOURCE_URL,BASE_SOURCE_SHA,base_zip)
    base_dir=pathlib.Path('_base_source_112');clean(base_dir);base_dir.mkdir()
    with zipfile.ZipFile(io.BytesIO(base)) as z:
        assert z.testzip() is None; z.extractall(base_dir)

    stage=pathlib.Path('_stage_112');clean(stage);shutil.copytree(base_dir,stage)
    ...
    # Reuse the exact complete source of the published 1.1.0 release.
    shutil.copytree(ROOT/'overrides',stage,dirs_exist_ok=True)
    patch_version(stage)
    note=(ROOT/'ZMENY_1.1.2.txt').read_text(encoding='utf-8')
    (stage/'ZMENY_1.1.2.txt').write_text(note,encoding='utf-8')
    ...
    qa=pathlib.Path('releases/1.1.2/validation').resolve();qa.mkdir(parents=True,exist_ok=True)
    ...
    source_stage=pathlib.Path('_source_112');clean(source_stage);shutil.copytree(stage,source_stage)
    clean_python_cache(source_stage)
    source_data=zip_deterministic(source_stage)
    pathlib.Path('releases/1.1.2/TURTO_source_1.1.2.zip').write_bytes(source_data)

    version_file=pathlib.Path('_turto_reporting_112_version.txt')
    version_file.write_text(
        "VSVersionInfo(ffi=FixedFileInfo(filevers=(1,1,2,0),prodvers=(1,1,2,0),mask=0x3f,flags=0x0,OS=0x40004,fileType=0x1,subtype=0x0,date=(0,0)),kids=[StringFileInfo([StringTable('040504B0',[StringStruct('CompanyName','TURTO s.r.o.'),StringStruct('FileDescription','TURTO – Měsíční přehledy'),StringStruct('FileVersion','1.1.2'),StringStruct('InternalName','TURTO_Mesicni_Prehledy'),StringStruct('OriginalFilename','TURTO_Mesicni_Prehledy.exe'),StringStruct('ProductName','TURTO – Měsíční přehledy'),StringStruct('ProductVersion','1.1.2')])]),VarFileInfo([VarStruct('Translation',[1029,1200])])])\n",
        encoding='utf-8')

    dist_main=pathlib.Path('_dist_main_112').resolve();work_main=pathlib.Path('_work_main_112').resolve();spec_main=pathlib.Path('_spec_main_112').resolve()
    ...
    smoke_data=pathlib.Path('_smoke_data_112').resolve();clean(smoke_data)
    ...
    cp=subprocess.run([str(main_exe),'--turto-chart-self-test'],cwd=main_dir,env=env,timeout=180)
    assert cp.returncode==0,cp.returncode

    subprocess.run([sys.executable,str((ROOT/'shortcut_check.py').resolve()),str(stage.resolve()),str(main_exe),str(qa)],check=True,timeout=120)
    ...
    install_root=pathlib.Path('_installed_112').resolve();test_data=pathlib.Path('_installed_data_112').resolve()
    ...
    sys.path.insert(0,str(stage.resolve()))
    from src.windows_shell import com_scope,read_link,shell_folder,APP_NAME,same_target
    with com_scope():
        assert same_target(read_link(shell_folder(2)/APP_NAME/(APP_NAME+'.lnk')),install_root/MAIN_EXE)
    sys.path.pop(0)
    ...
    upgrade_root=pathlib.Path('_upgrade_112').resolve();upgrade_data=pathlib.Path('_upgrade_data_112').resolve()
    ...
    update_stage=pathlib.Path('_update_stage_112');clean(update_stage);update_stage.mkdir()
    ...
    rspec=importlib.util.spec_from_file_location('release_safety_112',stage/'src/release_safety.py')
    ...
    print('TURTO_1_1_2_OK', 'update=',len(update_data),'setup=',setup.stat().st_size)
