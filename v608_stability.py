# TURTO CRM 6.0.8 stability layer
# Re-asserts agreed UI behavior after all feature modules and completes recipient ranking.
import datetime, sqlite3

def apply(M):
    # ---------- persistent stronger theme palette ----------
    LIGHT={
        'status_active':('#b6d8f0','#0b3554'),
        'status_offer':('#9fddd5','#064d47'),
        'status_done':('#b0ddb9','#124c22'),
        'status_won':('#9fd8af','#10461f'),
        'status_wait':('#ffe08a','#5c4200'),
        'status_soon':('#ffbc78','#713400'),
        'status_late':('#f5a2a2','#7b1111'),
        'status_cancel':('#d6dbe0','#3e474f')}
    DARK={
        'status_active':('#326484','#f5fbff'),
        'status_offer':('#197b72','#f0fffd'),
        'status_done':('#3b794b','#f1fff4'),
        'status_won':('#2f7a47','#effff4'),
        'status_wait':('#856719','#fff4c0'),
        'status_soon':('#96511c','#fff0df'),
        'status_late':('#943b3b','#fff1f1'),
        'status_cancel':('#555f68','#f5f7f8')}
    ...
    def safe_size(win,w=980,h=800):
        try:
            win.update_idletasks();sw=win.winfo_screenwidth();sh=win.winfo_screenheight()
            ww=min(max(560,int(w)),max(560,sw-50));hh=min(max(500,int(h)),max(500,sh-80))
            win.geometry(f'{ww}x{hh}+{max(0,(sw-ww)//2)}+{max(0,(sh-hh)//2)}');win.resizable(True,True)
        except:pass
    ...
    def mivo(self,*a,**k):
        ...
        try:
            ...
            for iid in self.mivo_tree.get_children():
                ...
                # Whole row by business state only.
    ...
    def requests(self,*a,**k):
        ...
        try:
            ...
            for iid in self.request_tree.get_children():
                ...
                state=str(vals[0] if vals else '').casefold()
                if not state.startswith('ček'):continue
                # Current Poptáno column is index 2.
                if len(vals)>2:
                    raw=str(vals[2]).replace('⚠','').replace('!','').replace('•','').strip();dt=None
                    for fmt in ('%d.%m.%Y','%Y-%m-%d'):
                        try:dt=datetime.datetime.strptime(raw,fmt).date();break
                    ...
                    if dt:
                        age=(today-dt).days
                        vals[2]=('⚠  '+raw) if age>=11 else (('•  '+raw) if age>=6 else raw)
    ...
    def load_contacts(self,selected=None):
        ...
        for rr in rows:
            email=(rr['email'] or '').strip();with_email+=1 if email else 0
            label=(rr['name'] or '').strip() or '(bez jména)'
            if rr['role']:label+=f" · {rr['role']}"
            label+=f' — {email}' if email else ' — bez e-mailu'
            var=M.tk.BooleanVar(value=email.lower() in selected_set if email else False)
    ...
    def req_ok(self,*a,**k):
        try:
            cid=self.company_id();selected=[email for var,email in self.contact_vars if email and var.get()]
