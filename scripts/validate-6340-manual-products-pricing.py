...
class Module:
    ...
    def db(self):
        ...
        con.create_collation("CZECH", lambda a, b: (str(a or "").casefold() > str(b or "").casefold()) - (str(a or "").casefold() < str(b or "").casefold()))
...
def seed(M):
    with M.db() as con:
        con.executescript(
            ...
            CREATE TABLE actions(id INTEGER PRIMARY KEY,name TEXT,archived INTEGER DEFAULT 0);
            ...
            CREATE TABLE product_subgroups(
              ...
              sort_order INTEGER DEFAULT 10,default_margin_pct REAL DEFAULT 0,default_discount_pct REAL DEFAULT 0
            );
            ...
            CREATE TABLE catalog_product_sources(
              id INTEGER PRIMARY KEY AUTOINCREMENT,product_id INTEGER NOT NULL,supplier_company_id INTEGER,
              supplier_name TEXT DEFAULT '',supplier_name_norm TEXT DEFAULT '',source_key TEXT NOT NULL UNIQUE,
              product_identity TEXT NOT NULL,supplier_product_code TEXT DEFAULT '',source_name TEXT DEFAULT '',
              source_kind TEXT DEFAULT '',last_seen_at TEXT DEFAULT CURRENT_TIMESTAMP,
              created_at TEXT DEFAULT CURRENT_TIMESTAMP
            );
            CREATE TABLE price_lists(id INTEGER PRIMARY KEY,valid_from TEXT,valid_to TEXT,archived INTEGER DEFAULT 0);
            ...
            CREATE TABLE supplier_offer_items(id INTEGER PRIMARY KEY,offer_id INTEGER,catalog_product_id INTEGER);
            CREATE TABLE business_document_items(id INTEGER PRIMARY KEY,document_id INTEGER,discount_pct REAL DEFAULT 0);
...
def main():
    with tempfile.TemporaryDirectory(prefix="turto-6340-") as tmp:
        ...
        seed(M)
        pricing.ensure_schema(M)
        product_id = pricing.save_product(M, {
        ...
        with M.db() as con:
            ...
            assert con.execute("SELECT source_kind FROM catalog_product_sources WHERE product_id=?", (product_id,)).fetchone()[0] == "manual"
        ...
        base = pricing.resolve(M, product_id)
        ...
        company_rule = pricing.save_rule(M, product_id, 1, 8, note="Rámcová sleva")
        company = pricing.resolve(M, product_id, 1)
        assert company["discount_source"] == "Sleva společnosti"
        close(company["discount_pct"], 8)

        action_rule = pricing.save_rule(M, product_id, 1, 12, action_id=10, note="AKCE ALFA")
        assert action_rule > company_rule
        action = pricing.resolve(M, product_id, 1, 10)
        assert action["discount_source"] == "Výjimka pro Akci"
        close(action["discount_pct"], 12)
        close(pricing.resolve(M, product_id, 1, 20)["discount_pct"], 8)
        close(pricing.resolve(M, product_id, 2, 10)["discount_pct"], 5)

        pricing.save_rule(M, product_id, 1, 9)
        with M.db() as con:
            assert con.execute(
            ...
            con.execute("INSERT INTO price_list_items VALUES(1,1,?,80,'CZK','ks',1)", (product_id,))

        imported = pricing.resolve(M, product_id, 1, 10, "2026-08-31")
        assert imported["purchase_source"] == "Ceník"
        close(imported["purchase_unit_price"], 80)
        close(imported["discount_pct"], 12)

        payload = {
            "company_id": 1,"action_id": 10,"issue_date": "2026-08-31",
            "items": [{"catalog_product_id": product_id,"quantity": 2,"purchase_unit_price": 0,"margin_pct": 25,"discount_pct": 5}],
        }
        priced = pricing.price_payload(M, payload)
        item = priced["items"][0]
        ...
        manual = {
            "company_id": 1,"action_id": 10,"issue_date": "2026-08-31",
            "items": [{"catalog_product_id": product_id,"quantity": 2,"purchase_unit_price": 80,"margin_pct": 25,"discount_pct": 7,"discount_source": "Ruční"}],
        }
        manual_item = pricing.price_payload(M, manual)["items"][0]
        close(manual_item["discount_pct"], 7)
        close(manual_item["unit_price"], 93)

        entry = (ROOT / "crm_price_lists.py").read_text(encoding="utf-8")
        assert "install_customer_pricing(module)" in entry
        assert entry.index("_apply_price_lists(module)") < entry.index("install_customer_pricing(module)")

    print("OK 6.3.40: manual products, Ceník precedence and Action/company discounts")
