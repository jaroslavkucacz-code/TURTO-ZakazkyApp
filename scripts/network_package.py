...
for name in ('TURTO-CRM-Sitovy-Pilot.exe', 'TURTO-CRM-Pilot-Admin.exe', 'TURTO-CRM-Mistni-Ukazka.exe',
             'TURTO-CRM-Kontrola-Pripojeni.exe',
...
assert all(local_result[key] for key in ('ok', 'frozen', 'editor', 'reader', 'concurrent_conflict',
    ...
    'accented_install_path', 'failure_diagnostic'))
check = json.loads((ROOT / 'artifacts/network/windows/network-check.json').read_text(encoding='utf-8'))
assert all(check[key] for key in ('ok', 'frozen', 'open_closed_ports', 'copy', 'invalid_path_rejected'))
...
manifest = {'pilot_version': '0.3.1', 'source_commit': subprocess.check_output(['git', 'rev-parse', 'HEAD'], cwd=ROOT, text=True).strip(),
...
archive = output / 'TURTO_CRM_Pilot_s_Mistni_Ukazkou_0.3.1_Windows_x64.zip'
