...
for name in ('TURTO-CRM-Sitovy-Pilot.exe', 'TURTO-CRM-Pilot-Admin.exe', '_internal/network_db/demo.db', '_internal/network_db/directory.sql'):
...
manifest = {'pilot_version': '0.2.0', 'source_commit': subprocess.check_output(['git', 'rev-parse', 'HEAD'], cwd=ROOT, text=True).strip(),
            'kind': 'standalone-network-pilot', 'production_updater': False, 'company_data': 'artificial-demo-only',
            'frozen_tls_login_edit_read_test': 'passed'}
(bundle / 'pilot-version.json').write_text(json.dumps(manifest, indent=2), encoding='utf-8')
archive = output / 'TURTO_CRM_Sitovy_Pilot_0.2.0_Windows_x64.zip'
