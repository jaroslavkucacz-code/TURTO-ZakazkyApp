...
for f in \
  ...
  v760_table_activity_performance.py v767_offer_reprocess_images.py post_baseline.py
do
...
grep -q "crm_features.install_offer_ui(app);crm_price_lists.apply(app);v710_cleanup.apply(app);v720_visual_offer.apply(app);v730_polish.apply(app);v740_offer_defaults.apply(app);v750_context_filters_offer_format.apply(app);v760_table_activity_performance.apply(app);v767_offer_reprocess_images.apply(app)" "$DIR/ZakazkyCRM.pyw"
...
grep -q "reprocessed_existing" "$DIR/_runtime/v767_offer_reprocess_images.py"
grep -q "DELETE FROM supplier_offer_items WHERE offer_id" "$DIR/_runtime/v767_offer_reprocess_images.py"
grep -q "Gerotop_Parser_767 as gerotop" "$DIR/offers_engine/Nabidky_Router.py"
grep -q "def _extract_row_image" "$DIR/offers_engine/Gerotop_Parser_767.py"
grep -q "insert_image" "$DIR/offers_engine/Gerotop_Parser_767.py"
...
assert text.index('v760_table_activity_performance.apply(app)') < text.index('v767_offer_reprocess_images.apply(app)')
PY
python - "$DIR" <<'PY'
import pathlib,sys
