...
def open_admin(app,auth=False):
    ...
    ttk.Label(hist,text="Auditní HISTORIE",style="Section.TLabel").pack(anchor="w");cols=("Čas","Uživatel","PC","Objekt","Akce","Pole","Původní","Nová");t=ttk.Treeview(hist,columns=cols,show="headings", name='layout__crm_runtime__open_admin__t');[t.heading(x,text=x) for x in cols];t.pack(fill="both",expand=True)
