...
def build_settings(self):
    from price_lists_domain.platform.settings_layout import build_shell, card, label, action
    build_shell(self)
    f = card(self, "general")
    label(f, "Vzhled", heading=True)
    self.theme = tk.StringVar(value=M.get_user_setting(self.active_user.get(), "theme", "Světlý"))
    cb = M.safe_combobox(f, textvariable=self.theme, values=["Světlý", "Tmavý"], state="readonly")
    cb.grid(row=1, column=0, sticky="w", pady=4)
    cb.bind("<<ComboboxSelected>>", lambda e: self.apply_theme(self.theme.get(), True))
    f = card(self, "general")
    label(f, "Uživatelé", heading=True)
    action(f, "Spravovat uživatele…", self.manage_users, 1)
    action(f, "Vytvořit zástupce TURTO CRM", self.create_desktop_shortcut, 2)
    f = card(self, "general")
    label(f, "Číselníky", heading=True)
    label(f, "Funkce osob · Poptávané zboží · Co se řeší · Obchodníci", 1)
    action(f, "Spravovat číselníky…", self.manage_code_lists, 2)
    f = card(self, "data")
    label(f, "Zálohování a kontrola dat", heading=True)
    label(f, f"Databáze: {M.DB}", 1)
    action(f, "Vytvořit zálohu", self.manual_backup, 2)
    action(f, "Zkontrolovat data", self.database_audit, 3)
    action(f, "Zálohy a úklid úložiště…", self.open_storage_maintenance, 4)
    f = card(self, "data")
    label(f, "Import / export", heading=True)
    action(f, "Export kompletní databáze…", self.export_complete_data, 1)
    action(f, "Import kompletní databáze…", self.import_complete_data, 2)
    action(f, "Výběrový export…", self.export_selected_dialog, 3)
    f = card(self, "maintenance")
    label(f, "Aktualizace aplikace", heading=True)
    M.set_setting("update_source", GITHUB_UPDATE)
    self.update_source = tk.StringVar(value=GITHUB_UPDATE)
    self._turto_windows_channel_label = label(f, f"TURTO CRM {getattr(M, 'APP_VERSION', '')} · stabilní kanál · automatická kontrola aktualizací", 1)
    action(f, "Zkontrolovat aktualizace", self.check_for_updates, 2, accent=True)
    label(f, "Aktualizace se stahují automaticky z GitHubu. Databáze ani firemní data se na GitHub neodesílají.", 3)
