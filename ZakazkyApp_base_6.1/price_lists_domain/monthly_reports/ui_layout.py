...
class PageViewport(tk.Frame):
    def __init__(self,master):
        ...
        self._wheel_bindings = {event:self._wheel_root.bind(event,self._wheel,add='+')
                                for event in ('<MouseWheel>', '<Button-4>', '<Button-5>')}

    def destroy(self):
        for event, token in self._wheel_bindings.items():
            if token:self._wheel_root.unbind(event, token)
