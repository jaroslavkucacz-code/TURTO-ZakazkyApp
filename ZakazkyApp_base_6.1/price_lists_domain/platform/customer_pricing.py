...
imported price. Customer and Action discounts stay outside price lists and issued
documents keep immutable line snapshots.
"""
...
def ensure_customer_pricing_schema(M) -> None:
    """Apply the additive 6.3.40 schema."""
    with M.db() as con:
        for declaration in (
...
def save_manual_product(M, values: dict, product_id=None) -> int:
    """Create or update a stable product without creating a fake Ceník."""
    ...
    with M.db() as con:
        ...
        con.execute(
            ...
            (product_id, manufacturer, manufacturer.casefold(), identity, identity, code, name),
        )
...
def apply_pricing_to_document_payload(M, payload: dict, force: bool = False) -> dict:
    ...
    for item in result.get("items") or []:
        ...
        source_value = str(item.get("discount_source") or "").casefold()
        may_replace = (
...
def open_manual_product_editor(M, app, parent, product_id=None, on_saved=None):
    ...
    for item in subgroups:
        subgroup_by_group.setdefault(int(item["category_id"]), []).append(item)
    ...
    group_name = next((str(item["name"]) for item in groups if int(item["id"]) == category_id), "Nezařazeno")
    ...
    def refresh_subgroups(*_):
        ...
        labels = ["Bez podskupiny"] + [str(item["name"]) for item in subgroup_rows]
        subgroup_combo.configure(values=labels)
        selected = next((str(item["name"]) for item in subgroup_rows if subgroup_id and int(item["id"]) == subgroup_id), None)
...
def open_discount_rules(M, app, parent, product_id: int, on_changed=None):
    ...
    def refresh():
        ...
        for item in data:
            ...
            tree.insert("", "end", iid=iid, values=(
                item["company_name"], item["action_name"] or "Výchozí pro společnost",
                f"{_number(item['discount_pct']):g} %", item["note"] or "",
    ...
    def edit_rule(existing=None):
        ...
        company_label = next((label for label, cid in company_map.items() if existing and cid == existing.get("company_id")), companies[0])
    ...
    def remove():
        ...
        if not M.messagebox.askyesno(
            ...
            return
        delete_discount_rule(M, item["id"])
        refresh()
        if callable(on_changed):
            on_changed()
    ...
    M.ttk.Button(tools, text="Upravit", command=edit_selected).pack(side="left", padx=5)
...
def open_manual_products_manager(M, app, parent):
    ...
    def refresh(*_):
        ...
        for item in data:
            iid = f"p{item['id']}"
            rows[iid] = dict(item)
            manual_price = item["manual_purchase_unit_price"]
            price_text = "" if manual_price is None else f"{_number(manual_price):,.2f} {item['manual_currency'] or 'CZK'}".replace(",", " ")
            margin = "dědí" if item["default_margin_pct"] is None else f"{_number(item['default_margin_pct']):g} %"
            discount = "dědí" if item["default_discount_pct"] is None else f"{_number(item['default_discount_pct']):g} %"
            placement = item["category_name"] + (f" › {item['subgroup_name']}" if item["subgroup_name"] else "")
            tree.insert("", "end", iid=iid, values=(
                "Ruční" if item["manual_product"] else "Import", item["internal_code"] or "",
                item["internal_name"] or "", item["manufacturer_name"] or "", placement,
                price_text, item["manual_unit"] or "ks", margin, discount,
                "Ano" if item["active"] else "Ne",
            ))
...
def _patch_product_workspace(M) -> None:
    ...
    def catalog_rows_proxy(module, scope, query="", manufacturer="", show_inactive=False,
                           limit=250, offset=0, sort_mode="Skupina → podskupina → produkt"):
        return _catalog_rows_with_manual(
            module, product_workspace, scope, query, manufacturer,
            show_inactive, limit, offset, sort_mode,
        )

    product_workspace._catalog_rows = catalog_rows_proxy
