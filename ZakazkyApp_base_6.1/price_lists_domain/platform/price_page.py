...
def _filter_values(M, app):
    ...
    result = (suppliers, groups, [row["name"] for row in categories.list_categories(M)])
    app._price_filter_cache = (time.monotonic(), result)
    return result
...
def build_price_lists(M, app) -> None:
    ...
    app.price_page = 0
    labels = ("Hledat produkt / kód", "Dodavatel", "Kategorie", "Produktová skupina", "Cena platná k datu", "Řádků")
    ...
    app.price_group_box.grid(row=1, column=3, sticky="ew", padx=(0, 5))
    M.DatePicker(filters, app.price_effective_date).grid(row=1, column=4, sticky="ew", padx=(0, 5))
    ...
    current_cols = (
        "Kategorie", "Dodavatel", "Větev", "Kód", "Produkt", "Cena/MJ", "Zdrojová cena",
        "Cena za", "MJ", "Přirážka/Sleva", "Hmotnost/MJ", "Min. odběr", "Podmínka", "Platí od", "Zdrojový ceník",
    )
    current_widths = [210, 180, 180, 120, 340, 115, 115, 75, 65, 115, 105, 95, 230, 95, 240]
    ...
    for col, label in enumerate(("Hledat", "Dodavatel", "Kategorie", "Stav")):
    ...
    M.ttk.Button(evidence_tools, text="Přiřadit kategorii", command=lambda: _assign_list_category(M, app)).pack(side="right", padx=3)
    ...
    evidence_cols = (
        "Stav", "Platí od", "Platí do", "Dodavatel", "Kategorie", "Název", "Skupina", "Větev",
    ...
    for variable in (
        ...
        app.price_group_filter, app.price_effective_date, app.price_page_size,
    ):
...
def _refresh_current(M, app, allow_fts_retry: bool = True) -> None:
    ...
    sql = f"""
        ...
        ORDER BY category COLLATE CZECH,supplier COLLATE CZECH,name COLLATE CZECH,item_id
        LIMIT ? OFFSET ?
    """
    try:
        with M.db() as con:
    ...
    for index, row in enumerate(rows, 1):
        ...
        tree.insert(
            "", "end", iid=iid,
            values=(
                row["category"], row["supplier"], row["branch"] or "", row["product_code"] or row["item_key"] or "",
